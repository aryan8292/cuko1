...
from database.ia_filterdb import Media, get_file_details, unpack_new_file_id, get_bad_files, Media2, get_file_details, unpack_new_file_id
from database.users_chats_db import db
from info import CHANNELS, ADMINS, AUTH_CHANNEL, LOG_CHANNEL, PICS, BATCH_FILE_CAPTION, CUSTOM_FILE_CAPTION, PROTECT_CONTENT, CHNL_LNK, GRP_LNK, REQST_CHANNEL, SUPPORT_CHAT_ID, SUPPORT_CHAT, MAX_B_TN, VERIFY, SHORTLINK_API, SHORTLINK_URL, TUTORIAL, IS_TUTORIAL, PREMIUM_USER, MOVIE_GROUP, SERIES_GROUP, MAIN_CHANNEL, VIP_MEMBERSHIP
...
async def start(client, message):
    ...
    if AUTH_CHANNEL and not await is_subscribed(client, message):
        ...
        if message.command[1] != "subscribe":
            try:
                ...
                btn.append([InlineKeyboardButton('🔆 Ary_moviex 🔆', url=MAIN_CHANNEL)])
    ...
    if data.startswith("sendfiles"):
        ...
        k = await client.send_message(chat_id=user,text=f"<b>📕 Fɪʟᴇ Nᴀᴍᴇ ➠ : [{files.file_name}](https://t.me/+SOC2c_33ZlvvyPuO) \n\n📙 Fɪʟᴇ Sɪᴢᴇ ➠ : {get_size(files.file_size)}\n\n📗 Dᴏᴡɴʟᴏᴀᴅ Lɪɴᴋ ➠ : {g} </b>", disable_web_page_preview=True, reply_markup=InlineKeyboardMarkup(
                [
                    [
                        InlineKeyboardButton('🌀 Dᴏᴡɴʟᴏᴀᴅ Lɪɴᴋ 🌀', url=g)
                    ], [
        ...
        if settings['is_shortlink'] and user not in PREMIUM_USER:
            ...
            files = files_[0]
            g = await get_shortlink(chat_id, f"https://telegram.me/{temp.U_NAME}?start=file_{file_id}")
            k = await client.send_message(chat_id=message.from_user.id,text=f"<b>📕 Fɪʟᴇ Nᴀᴍᴇ ➠ : [{files.file_name}](https://t.me/+SOC2c_33ZlvvyPuO) \n\n📙 Fɪʟᴇ Sɪᴢᴇ ➠ : {get_size(files.file_size)}\n\n📗 Dᴏᴡɴʟᴏᴀᴅ Lɪɴᴋ ➠ : {g} </b>", disable_web_page_preview=True, reply_markup=InlineKeyboardMarkup(
                    [
                        [
                            InlineKeyboardButton('🌀 Dᴏᴡɴʟᴏᴀᴅ Lɪɴᴋ 🌀', url=g)
                        ], [
