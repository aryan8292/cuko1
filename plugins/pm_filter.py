...
from info import ADMINS, AUTH_CHANNEL, AUTH_USERS, SUPPORT_CHAT_ID, CUSTOM_FILE_CAPTION, MSG_ALRT, PICS, AUTH_GROUPS, P_TTI_SHOW_OFF, GRP_LNK, CHNL_LNK, NOR_IMG, LOG_CHANNEL, SPELL_IMG, MAX_B_TN, IMDB, \
    SINGLE_BUTTON, SPELL_CHECK_REPLY, IMDB_TEMPLATE, NO_RESULTS_MSG, TUTORIAL, REQST_CHANNEL, IS_TUTORIAL, QUALITIES, LANGUAGES, SEASONS, SUPPORT_CHAT, PREMIUM_USER, AUTO_REQUEST, MOVIE_GROUP, SERIES_GROUP, MAIN_CHANNEL
...
async def pm_text(bot, message):
    ...
    await message.reply_text(
         ...
         reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔆 Request Group", url=f"https://t.me/Ary_moviex/2103")]])
    )
...
async def next_page(bot, query):
    ...
    if settings["button"]:
        cap = f"<b>🔆 Hᴇʏ {query.from_user.mention}, I Fᴏᴜɴᴅ {total} Rᴇsᴜʟᴛs Fᴏʀ➠ {search}👇🏻\n\n🗨 Sᴇʟᴇᴄᴛ A Lɪɴᴋ & Pʀᴇss Sᴛᴀʀᴛ ↷\n\n</b>"
    else:
...
async def filter_languages_cb_handler(client: Client, query: CallbackQuery):
    ...
    try:
        if int(req) not in [query.message.reply_to_message.from_user.id, 0]:
            return await query.answer(
                f"⚠️ Hᴇʟʟᴏ{query.from_user.first_name},\Tʜɪꜱ Iꜱ Nᴏᴛ Yᴏᴜʀ Mᴏᴠɪᴇ Rᴇǫᴜᴇꜱᴛ,\nRᴇǫᴜᴇꜱᴛ Yᴏᴜʀ'ꜱ...",
                show_alert=False,
            )
    except:
        pass
...
async def filter_qualities_cb_handler(client: Client, query: CallbackQuery):
    ...
    if offset != "":
        ...
        btn.append(
            [InlineKeyboardButton(text="⌫ 𝐍𝐎 𝐌𝐎𝐑𝐄 𝐏𝐀𝐆𝐄𝐒 𝐀𝐕𝐀𝐋𝐈𝐀𝐁𝐋𝐄 ⌫",callback_data="pages")]
        )
    # if ENABLE_SHORTLINK == True:
    btn.insert(0, [
        InlineKeyboardButton("🔆 Sᴇɴᴅ Aʟʟ Fɪʟᴇs 🔆", callback_data=f"sendfiles#{key}")
